import { hospitalById, stationById, vehicles as allVehicles, balancing } from '../data/index.ts'
import type { Station, Vehicle } from '../data/schemas.ts'
...
/**
 * Vehicle runtime simulation (GAME_DATA §10 lifecycle, §6/§7 duty times,
 * §10b Sonderstatus, §12b NEF-101 rule). UI-free and deterministic (seeded RNG).
 */
...
export class VehicleSim {
  ...
  private rng: Rng
  version = 0
  onEvent?: (e: VehicleEvent) => void

  constructor(seed = 42, vehicleList: Vehicle[] = allVehicles) {
    this.rng = mulberry32(seed)
    for (const v of vehicleList) {
      const st = stationById.get(v.homeStation)
      if (!st) continue
      this.runtimes.set(v.funkrufname, {
        id: v.funkrufname,
        vehicle: v,
        homeStation: st,
        status: 'AUS',
        basePos: { lat: st.lat, lon: st.lon },
        manualService: false,
      })
    }
  }
  ...
  private setStatus(rt: VehicleRuntime, to: VehiclePhase, simSec: number, note?: string) {
    ...
    rt.status = to
    rt.note = note
    this.emit({ simSec, vehicleId: rt.id, type: 'status', from, to, note })
  }
  ...
  private turnoutSec(rt: VehicleRuntime, simSec: number): number {
    if (rt.status !== '00') return FOLLOWUP_TURNOUT_SEC
    const t = balancing.turnout
    // GAME_DATA §12b: NEF 101 at night is staffed by hospital personnel
    if (rt.vehicle.specialRule === 'nef101' && isNight(simSec)) return t.khPersonalNachtSec
    const night = isNight(simSec)
    switch (rt.homeStation.staffing) {
      case 'hauptamtlich':
  ...
  /** Dispatch an assignment. Returns false when the vehicle cannot take it. */
  ...
  /** Send an available vehicle to a standby position (status 88 → 08/09/10). */
  ...
  cancelAssignment(id: string, simSec: number): boolean {
    ...
    this.emit({ simSec, vehicleId: rt.id, type: 'status', from, to: '6', note: 'Einsatzabbruch' })
    this.notify()
    return true
  }

  /** Manual Sonderstatus (91–95) for an available vehicle, with block time. */
  ...
  setReserveActive(id: string, active: boolean, simSec: number): boolean {
    const rt = this.runtimes.get(id)
    if (!rt || !rt.vehicle.reserve) return false
  ...
  private isInService(rt: VehicleRuntime, simSec: number, ctx: DutyContext): boolean {
    if (rt.vehicle.reserve) return rt.manualService
    return isOnDuty(rt.vehicle, simSec, ctx)
  }
  ...
  private tickVehicle(rt: VehicleRuntime, simSec: number, ctx: DutyContext): boolean {
    ...
    if (rt.status === 'AUS') {
      if (onDuty) {
        rt.basePos = { lat: rt.homeStation.lat, lon: rt.homeStation.lon }
        rt.status = '00'
        this.emit({ simSec, vehicleId: rt.id, type: 'spawn' })
        // Fahrzeugcheck at shift start (GAME_DATA §10b status 92)
        if (this.rng() < FAHRZEUGCHECK_PROBABILITY) {
    ...
    if (!onDuty && (rt.status === '00' || rt.status === '88' || isPositionCode(rt.status))) {
      // end of duty: vehicle leaves service (engaged vehicles finish first)
      this.endMove(rt)
      rt.positionTargetCode = undefined
      rt.positionTargetPos = undefined
    ...
    switch (rt.status) {
      ...
      case '6':
        if (rt.phaseUntil !== undefined && simSec >= rt.phaseUntil) {
          ...
          this.startMove(rt, { lat: rt.homeStation.lat, lon: rt.homeStation.lon }, simSec, false)
        }
        break
      case '7':
        if (rt.moveArrive !== undefined && simSec >= rt.moveArrive) {
